...
def _create_data(n_batch, n_time, n_feature, network, dtype=tf.int32, offset=0):
  ...
  src_size_placeholder = {0: tf.reshape(tf.range(n_time-n_batch+1, n_time+1, dtype=tf.int32), [n_batch])}  # varying seq_lens
  src = InternalLayer(name="src",
                      network=network,
                      output=Data(
                        name="src_output",
                        shape=(None, n_feature),
                        placeholder=src_placeholder,
                        dim=n_feature,
                        size_placeholder=src_size_placeholder))
  return src


def test_BERTMaskLayer_mono():
...
def _test_ProbabilisticMaskLayer(bil=False, with_prob=True, n_batch=5, n_time=20, n_feature=1,
...
def test_mask_and_gather(bil=False, with_prob=False, n_batch=10, n_time=11, n_feature=30):
...
def test_GatherPositionsLayer(n_batch = 3, n_time = 10, n_feature = 8, n_mask = 4):
  print("Test mask layer with "
  ...
  with make_scope() as session:
    ...
    pos_size_placeholder = {0: tf.reshape(tf.range(n_mask-n_batch+1, n_mask+1, dtype=tf.int32), (n_batch,))}  # varying seq_lens
    pos = InternalLayer(
    ...
    # check the results:
    print("Check if correct part of every sequence was gathered:", end=" ")
    for i in range(out_res.shape[0]):
      for j in range(out_lens_res[i]):
        assert np.alltrue(out_res[i,j] == src_res[i,positions_res[i,j]]), \
          "Position (%s,%s) not gathered correctly!" %(i,j)
